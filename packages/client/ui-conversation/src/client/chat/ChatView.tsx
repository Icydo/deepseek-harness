...
function TurnStatus({ startTime, t }: {
  ...
  // Per-character "diving" wave: each glyph owns one phase-shifted loop, so
  // the label reads as letters taking turns plunging into thought. The 120ms
  // stagger means a 13-glyph label spans ~1.4s of one 1.6s cycle — a
  // continuous ripple with no whole-label pulse.
  const glyphs = 'Deep diving...'
  return (
    <div className={css.turnStatus} role="status" aria-live="polite">
      {[...glyphs].map((glyph, index) => (
        <span
          key={index}
          className={css.turnChar}
